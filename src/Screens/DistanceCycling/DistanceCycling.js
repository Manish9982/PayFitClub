...
const DistanceCycling = ({ navigation, route }) => {
    ...
    const onPressNextSecond = async () => {
        var formdata = new FormData()
        formdata.append("token", Apis.TOKEN);
        formdata.append("caseid", Apis.insert_Cycling_case_id);
        formdata.append("userids", await throwUserID());
        formdata.append("time_key", route?.params?.frequency);
        formdata.append("cyclingtime", route?.params?.duration);
        formdata.append("cyclingdistance", steps?.trim());
        const result = await PostApiData(formdata)
        if (result?.status) {
            Alert.alert('Success', result?.message)
            setGoalId(result?.goal_id)
            navigation.navigate('StepCounterCycling', { footSteps: `${steps?.trim()}` })
        }
    }
    ...
    return (
        <KeyboardAwareScrollView>
            <View style={styles.container}>
                ...
                <Text style={styles.fonts}>For eg. If you selected the 'Day', and time duration as '1' in the previous step and you want to set the goal of 10 kms of cycling in '1' day so here you input 10.</Text>
                <TextInputMed
...
export default DistanceCycling
